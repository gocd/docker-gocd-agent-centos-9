...
# add user to root group for GoCD to work on openshift
  useradd -u ${UID} -g root -d /home/go -m go && \
    yum install --assumeyes glibc-langpack-en epel-release && \
  yum update -y && \
  yum install -y git mercurial subversion openssh-clients bash unzip procps procps-ng coreutils-single curl-minimal && \
  yum clean all && \
